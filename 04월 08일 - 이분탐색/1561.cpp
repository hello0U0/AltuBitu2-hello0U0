...
ll finalChildren(ll t, vector<int>& rides) {
    ll child = 0;
    for (int i = 1; i < rides.size(); i++) {        //해당시간동안 각 놀이기구를 타는 아이 수
        child += (t / rides[i] + 1);
    }
    return child;
}
...
ll lowerSearch(ll left, ll right, ll target, vector<int>& rides) {
    while (left <= right) {
        ll mid = (left + right) / 2;
        ll child = finalChildren(mid, rides);       //mid 시간동안 놀이기구를 타는 모든 아이의 수

        if (child >= target) { //줄의 마지막 아이보다 더 뒤에 있는 아이라면 앞쪽 탐색
            right = mid - 1;
        }
        else {                  //아니라면 뒤쪽 탐색
            left = mid + 1;
...
int findRides(int n, int m, vector<int>& rides) {
    ll t = lowerSearch(0, (ll)rides[1] * n, n, rides);      //n번째 아이가 놀이기구를 타는 첫 시간
    ll child = finalChildren(t, rides);                     //해당 시간동안 놀이기구를 타는 아이 수
    for (int i = m; i >= 1; i--) {
        if (t % rides[i] == 0) { //아이가 탐 
            if (child-- == n) {     //n명의 아이가 남으면 번호를 리턴
                return i;
            }
        }
    }
...
int main() {
    ...
    //연산 & 출력
    cout << findRides(n, m, rides);     //마지막 아이가 타는 놀이기구 번호
    return 0;
}