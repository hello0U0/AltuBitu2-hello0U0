...
int fillTile(int n) {                                               //
    ...
    if (n == 1) { //인덱스 에러 안나도록 n이 1일 경우 바로 리턴     // 너비가 1일때 백트래킹을 끝낸다.
        return 1;                                                   //
    }                                                               //
    ...
    dp[1] = 1;                                                      // 너비가 1일때
    dp[2] = 3;                                                      // 너비가 2일때
    for (int i = 3; i <= n; i++) {                                  // 너비가 3 이상이면
        dp[i] = dp[i - 1] + 2 * dp[i - 2];                          // 너비는 너비 1의 직사각형과 너비 n-1의 직사각형이 붙어 있거나, 너비 2의 직사각형(너비 1 직사각형 2개가 붙은 경우 제외)과 너비 n-2의 직사각형이 붙어있다.
