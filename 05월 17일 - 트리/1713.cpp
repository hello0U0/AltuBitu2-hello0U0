...
bool cmp(const pair<int, ci>& c1, const pair<int, ci>& c2) {
    if (c1.second.first != c2.second.first) {           //추천횟수가 적은 것을 리턴
        return c1.second.first < c2.second.first;
    }
    return c1.second.second < c2.second.second;         //추천횟수가 같으면 게시시간이 작은 것을 리턴
}
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (candidate.size() == n && candidate.find(input) == candidate.end()) {
            candidate.erase(min_element(candidate.begin(), candidate.end(), cmp));      //추천횟수가 적은, 게시시간이 작은 것을 지운다.
        }
        //첫 게시라면
        if (candidate.find(input) == candidate.end()) {     
            candidate[input].second = i;            //게시시간을 기록
        }
