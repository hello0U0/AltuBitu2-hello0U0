...
ci dfs(int node, int parent, vector<vector<ci>>& graph) {
    ...
    for (int i = 0; i < graph[node].size(); i++) {//노드와 연결된 모든 노드에 대해
        int next_node = graph[node][i].first;   //다음 노드
        if (next_node == parent) {     //다음 노드가 부모면 넘어간다.
            continue;
        }
        ...
        if (graph[node][i].second + dfs_search.second > len) { //기존 거리보다 길다면 갱신
            ...
            pos = dfs_search.first;     //해당 거리의 노드
        }
    }
    return { pos, len };    //가장 먼 거리의 노드, 길이탐색
}
...
int main() {
    ...
    for (int i = 1; i < n; i++) { //무방향 그래프로 만들기
        cin >> p >> c >> w;
        graph[p].push_back({ c, w });   // p->c 가중치 w
        graph[c].push_back({ p, w });   // c->p 가중치 w
    }
